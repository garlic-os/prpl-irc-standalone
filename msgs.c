...
static void IRCBuddy_status(char *name, IRCBuddy *ib, IRCConn *irc);
static void IRCConnected(IRCConn *irc, const char *nick);

static void irc_msg_handle_privmsg(
	IRCConn *irc,
	const char *name,
	const char *from,
	const char *to,
...
#ifdef HAVE_CYRUS_SASL
static void irc_sasl_finish(IRCConn *irc);
#endif
...
static void IRCConnected(IRCConn *irc, const char *nick) {
	...
	if (purple_status_type_get_primitive(purple_status_get_type(status)) !=
	...
	if (!irc->timer) {
		irc->timer = purple_timeout_add_seconds(
			45, (GSourceFunc) irc_blist_timeout, (gpointer) irc
		);
	}
}
...
void irc_msg_default(
	...
	for (cur = args[0], i = 0; i < 4; i++) {
		...
		if (i == 1) {
			if (end - cur != 3 || !isdigit(cur[0]) || !isdigit(cur[1]) ||
				!isdigit(cur[2])) {
				goto undirected;
			}
	...
	convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_ANY, convname, irc->account
	);
	g_free(convname);
...
void irc_msg_ban(
	...
	convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_CHAT, args[1], irc->account
	);
	...
	if (purple_strequal(name, "367")) {
		...
		if (args[3] && args[4]) {
			...
			msg = g_strdup_printf(
				_("Ban on %s by %s, set %s ago"), args[2], args[3], time
			);
			g_free(time);
		} else {
...
void irc_msg_banfull(
	...
	purple_conv_chat_write(
		...
		"",
		buf,
		(PurpleMessageFlags) (PURPLE_MESSAGE_SYSTEM | PURPLE_MESSAGE_NO_LOG),
		time(NULL)
	);
	g_free(buf);
}
...
void irc_msg_chanmode(
	...
	buf = g_strdup_printf(
		"mode for %s: %s %s", args[1], args[2], escaped ? escaped : ""
	);
	purple_conv_chat_write(
		PURPLE_CONV_CHAT(convo), "", buf, PURPLE_MESSAGE_SYSTEM, time(NULL)
	);
	g_free(escaped);
	g_free(buf);
...
void irc_msg_endwhois(
	...
	if (irc->whois.real) {
		purple_notify_user_info_add_pair_plaintext(
			user_info, _("Real name"), irc->whois.real
		);
		g_free(irc->whois.real);
	}
	if (irc->whois.login) {
		purple_notify_user_info_add_pair_plaintext(
			user_info, _("Login name"), irc->whois.login
		);
		g_free(irc->whois.login);
	}
	if (irc->whois.ident) {
		purple_notify_user_info_add_pair_plaintext(
			user_info, _("Ident name"), irc->whois.ident
		);
		g_free(irc->whois.ident);
	}
	if (irc->whois.host) {
		purple_notify_user_info_add_pair_plaintext(
			user_info, _("Host name"), irc->whois.host
		);
		g_free(irc->whois.host);
	}
	if (irc->whois.server) {
		tmp = g_strdup_printf(
			"%s (%s)", irc->whois.server, irc->whois.serverinfo
		);
	...
	if (irc->whois.channels) {
		purple_notify_user_info_add_pair(
			user_info, _("Currently on"), irc->whois.channels->str
		);
	...
	if (purple_strequal(irc->whois.nick, "Paco-Paco")) {
		purple_notify_user_info_add_pair(
			user_info, _("<b>Defining adjective:</b>"), _("Glorious")
		);
	}
...
void irc_msg_who(
	...
	if (purple_strequal(name, "352")) {
		...
		keys = g_list_prepend(keys, (char *) "userhost");
		values = g_list_prepend(values, userhost);

		keys = g_list_prepend(keys, (char *) "realname");
		...
		if (args[6][0] == 'G' && !(flags & PURPLE_CBFLAGS_AWAY)) {
			purple_conv_chat_user_set_flags(
				chat,
				cb->name,
				(PurpleConvChatBuddyFlags) (flags | PURPLE_CBFLAGS_AWAY)
			);
...
void irc_msg_list(
	...
	if (purple_strequal(name, "322")) {
		...
		if (!purple_roomlist_get_in_progress(irc->roomlist)) {
			purple_debug_warning(
				"irc", "Buggy server didn't send RPL_LISTSTART.\n"
			);
			purple_roomlist_set_in_progress(irc->roomlist, TRUE);
		}

		room = purple_roomlist_room_new(
			PURPLE_ROOMLIST_ROOMTYPE_ROOM, args[1], NULL
		);
		purple_roomlist_room_add_field(irc->roomlist, room, args[1]);
		purple_roomlist_room_add_field(
			irc->roomlist, room, GINT_TO_POINTER(strtol(args[2], NULL, 10))
		);
...
void irc_msg_topic(
	...
	convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_CHAT, chan, irc->account
	);
	if (!convo) {
		purple_debug(
			PURPLE_DEBUG_ERROR,
	...
	if (purple_strequal(name, "topic")) {
		const char *current_topic =
			purple_conv_chat_get_topic(PURPLE_CONV_CHAT(convo));
		if (!(current_topic != NULL && purple_strequal(tmp2, current_topic))) {
			...
			if (*tmp2) {
				msg = g_strdup_printf(
					_("%s has changed the topic to: %s"), nick_esc, tmp2
				);
			} else {
		...
		purple_conv_chat_write(
			PURPLE_CONV_CHAT(convo), "", msg, PURPLE_MESSAGE_SYSTEM, time(NULL)
		);
		g_free(msg);
	}
	g_free(tmp2);
...
void irc_msg_topicinfo(
	...
	msg = g_strdup_printf(
		_("Topic for %s set by %s at %s on %s"),
		args[1],
		args[2],
		timestamp,
		datestamp
...
void irc_msg_unknown(
	...
	purple_notify_error(
		...
		_("Unknown message"),
		buf,
		_("The IRC server received a message it did not understand.")
	);
	g_free(buf);
}
...
void irc_msg_names(
	...
	if (purple_strequal(name, "366")) {
		convo = purple_find_conversation_with_account(
			PURPLE_CONV_TYPE_ANY, args[1], irc->account
		);
		if (!convo) {
			purple_debug(
				PURPLE_DEBUG_ERROR,
		...
		if (purple_conversation_get_data(convo, IRC_NAMES_FLAG)) {
			msg = g_strdup_printf(
				_("Users on %s: %s"), args[1], names ? names : ""
			);
			...
			if (users != NULL) {
				...
				purple_conv_chat_add_users(
					PURPLE_CONV_CHAT(convo), users, NULL, flags, FALSE
				);
			...
			purple_conversation_set_data(
				convo, IRC_NAMES_FLAG, GINT_TO_POINTER(TRUE)
			);
		}
		g_free(names);
	} else {
...
void irc_msg_nonick(
	...
	if (convo) {
		if (purple_conversation_get_type(convo) ==
			PURPLE_CONV_TYPE_CHAT) { /* does this happen? */
			purple_conv_chat_write(
				PURPLE_CONV_CHAT(convo),
				args[1],
				_("no such channel"),
...
void irc_msg_notop(
	...
	purple_conv_chat_write(
		PURPLE_CONV_CHAT(convo), "", args[2], PURPLE_MESSAGE_SYSTEM, time(NULL)
	);
}
...
void irc_msg_ison(
	...
	for (i = 0; nicks[i]; i++) {
		if ((ib = (IRCBuddy *) g_hash_table_lookup(
				 irc->buddies, (gconstpointer) nicks[i]
			 )) == NULL) {
			continue;
		}
	...
	if (!irc->ison_outstanding) {
		g_hash_table_foreach(
			irc->buddies, (GHFunc) IRCBuddy_status, (gpointer) irc
		);
	}
}
...
void irc_msg_join(
	...
	if (!purple_utf8_strcasecmp(nick, purple_connection_get_display_name(gc))) {
		...
		g_free(nick);
		convo = purple_find_conversation_with_account(
			PURPLE_CONV_TYPE_CHAT, args[0], irc->account
		);
		...
		purple_conversation_set_data(
			convo, IRC_NAMES_FLAG, GINT_TO_POINTER(FALSE)
		);
	...
	if (convo == NULL) {
		purple_debug(
			PURPLE_DEBUG_ERROR, "irc", "JOIN for %s failed\n", args[0]
		);
		g_free(nick);
		return;
	}
	...
	if ((ib = (IRCBuddy *) g_hash_table_lookup(irc->buddies, nick)) != NULL) {
		ib->new_online_status = TRUE;
		IRCBuddy_status(nick, ib, irc);
	}
...
void irc_msg_kick(
	...
	if (!purple_utf8_strcasecmp(
		...
		buf = g_strdup_printf(
			_("You have been kicked by %s: (%s)"), nick, args[2]
		);
		purple_conv_chat_write(
			PURPLE_CONV_CHAT(convo),
			args[0],
		...
		g_free(buf);
		serv_got_chat_left(
			gc, purple_conv_chat_get_id(PURPLE_CONV_CHAT(convo))
		);
	} else {
...
void irc_msg_mode(
	...
	if (*args[0] == '#' || *args[0] == '&') { /* Channel	*/
		...
		buf = g_strdup_printf(
			_("mode (%s %s) by %s"), args[1], escaped ? escaped : "", nick
		);
		purple_conv_chat_write(
			PURPLE_CONV_CHAT(convo),
			args[0],
		...
		g_free(buf);
		if (args[2]) {
			int /*PurpleConvChatBuddyFlags*/ flags;
...
void irc_msg_nick(
	...
	while (chats) {
		PurpleConvChat *chat = (PurpleConvChat *) PURPLE_CONV_CHAT(
			(PurpleConversation *) chats->data
		);
		/* This is ugly ... */
	...
	conv = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_IM, nick, irc->account
	);
	if (conv != NULL) {
...
void irc_msg_badnick(
	...
	if (purple_connection_get_state(gc) == PURPLE_CONNECTED) {
		purple_notify_error(
			...
			_("Invalid nickname"),
			_("Invalid nickname"),
			_("Your selected nickname was rejected by the server.  It probably "
		...
		purple_connection_error_reason(
			...
			_("Your selected account name was rejected by the server.  It "
			  "probably contains invalid characters.")
		);
	}
}
...
void irc_msg_nickused(
	...
	if (gc && purple_connection_get_state(gc) == PURPLE_CONNECTED) {
		...
		buf = g_strdup_printf(
			_("The nickname \"%s\" is already being used."), irc->reqnick
		);
		purple_notify_error(
			gc, _("Nickname in use"), _("Nickname in use"), buf
		);
		g_free(buf);
		g_free(irc->reqnick);
		irc->reqnick = NULL;
...
void irc_msg_nochangenick(
	...
	g_return_if_fail(gc);
	purple_notify_error(
		gc, _("Cannot change nick"), _("Could not change nick"), args[2]
	);
}
...
void irc_msg_part(
	...
	convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_CHAT, channel, irc->account
	);
	if (!convo) {
		purple_debug(
			PURPLE_DEBUG_INFO,
	...
	if (!purple_utf8_strcasecmp(nick, purple_connection_get_display_name(gc))) {
		...
		msg = g_strdup_printf(
			_("You have parted the channel%s%s"),
			(args[1] && *args[1]) ? ": " : "",
...
void irc_msg_pong(
	...
	if (sscanf(parts[1], "%lu", (unsigned long *) &oldstamp) != 1) {
		msg = g_strdup(_("Error: invalid PONG from server"));
	} else {
		msg = g_strdup_printf(
			_("PING reply -- Lag: %lu seconds"), time(NULL) - oldstamp
		);
	}

	convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_ANY, parts[0], irc->account
	);
	g_strfreev(parts);
	if (convo) {
		if (purple_conversation_get_type(convo) == PURPLE_CONV_TYPE_CHAT) {
...
static void irc_msg_handle_privmsg(
	...
	if (!purple_utf8_strcasecmp(to, purple_connection_get_display_name(gc))) {
		serv_got_im(gc, nick, msg, (PurpleMessageFlags) 0, time(NULL));
	} else {
		convo = purple_find_conversation_with_account(
			PURPLE_CONV_TYPE_CHAT, irc_nick_skip_mode(irc, to), irc->account
		);
		if (convo) {
			serv_got_chat_in(
				gc,
...
void irc_msg_regonly(
	...
	msg = g_strdup_printf(
		_("Cannot join %s: Registration is required."), args[1]
	);
...
void irc_msg_quit(
	...
	if ((ib = (IRCBuddy *) g_hash_table_lookup(irc->buddies, data[0])) !=
		NULL) {
		ib->new_online_status = FALSE;
		IRCBuddy_status(data[0], ib, irc);
	}
	g_free(data[0]);
...
void irc_msg_unavailable(
	...
	PurpleConnection *gc = purple_account_get_connection(irc->account);
	purple_notify_error(
		gc, NULL, _("Nick or channel is temporarily unavailable."), args[1]
	);
}
...
static int irc_sasl_cb_log(void *context, int level, const char *message) {
...
static void irc_auth_start_cyrus(IRCConn *irc) {
	int ret = 0;
	char *buf;
	...
	if (!irc->gsc) {
		...
		plaintext =
			purple_account_get_bool(account, "auth_plain_in_clear", FALSE);
		if (!plaintext) {
	...
	do {
		...
		ret = sasl_client_new(
			"irc", irc->server, NULL, NULL, irc->sasl_cb, 0, &irc->sasl_conn
		);
		...
		if (ret != SASL_OK) {
			...
			purple_connection_error_reason(
				gc, PURPLE_CONNECTION_ERROR_OTHER_ERROR, tmp
			);
			g_free(tmp);
			return;
		}
		...
		sasl_setprop(
			irc->sasl_conn, SASL_AUTH_EXTERNAL, irc->account->username
		);
		sasl_setprop(irc->sasl_conn, SASL_SEC_PROPS, &secprops);

		ret = sasl_client_start(
			irc->sasl_conn,
			irc->sasl_mechs->str,
			NULL,
			NULL,
		...
		switch (ret) {
			...
			case SASL_NOMECH:
				purple_connection_error_reason(
					...
					_("SASL authentication failed: No worthy authentication "
					  "mechanisms found.")
				);
			...
			default:
				...
				if (irc->current_mech && *irc->current_mech) {
					char *pos;
					if ((pos =
							 strstr(irc->sasl_mechs->str, irc->current_mech))) {
						size_t index = pos - irc->sasl_mechs->str;
						g_string_erase(
							irc->sasl_mechs, index, strlen(irc->current_mech)
						);
...
void irc_msg_cap(
	...
	if (strncmp(args[1], "ACK", 4)) {
		const char *tmp =
			_("SASL authentication failed: Server does not support SASL "
			  "authentication.");
		purple_connection_error_reason(
			gc, PURPLE_CONNECTION_ERROR_AUTHENTICATION_IMPOSSIBLE, tmp
		);
...
void irc_msg_auth(IRCConn *irc, char *arg) {
	...
	ret = sasl_client_step(
		irc->sasl_conn, serverin, serverinlen, NULL, &c_out, &clen
	);
