...
static void
irc_do_mode(IRCConn *irc, const char *target, const char *sign, char **ops);
...
int irc_cmd_default(
	...
	PurpleConversation *convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_ANY, target, irc->account
	);
	char *buf;
...
int irc_cmd_ctcp(
	 ...
	 * actually, this shouldn't be done here but somewhere else since irc should
...
int irc_cmd_ctcp_action(
	...
	if (strncmp(msg, "/me ", 4) != 0) {
		...
		action = (char *) g_malloc(strlen(&msg[4]) + 10);
...
int irc_cmd_kick(
	...
	convo = purple_find_conversation_with_account(
		PURPLE_CONV_TYPE_CHAT, target, irc->account
	);
	if (!convo) {
		return 0;
	}
...
int irc_cmd_mode(
	...
	if (purple_strequal(cmd, "mode")) {
		...
		buf = irc_format(
			irc, "vnc", "MODE", purple_connection_get_display_name(gc), args[0]
		);
	} else {
		return 0;
	}
...
int irc_cmd_op(
	...
	if (purple_strequal(cmd, "op")) {
		...
		purple_debug(
			PURPLE_DEBUG_ERROR, "irc", "invalid 'op' command '%s'\n", cmd
		);
		return 0;
	}
...
int irc_cmd_part(
	...
	if (args[1]) {
		buf =
			irc_format(irc, "vc:", "PART", args[0] ? args[0] : target, args[1]);
	} else {
...
int irc_cmd_privmsg(
	...
	while (*end && *cur) {
		...
		if (end - cur > max_privmsg_arg_len) {
			/* this call is used to find the last valid character position in
			 * the first max_privmsg_arg_len bytes of the utf-8 message
			 */
...
int irc_cmd_quit(
	...
	if (!irc->quitting) {
		/*
		 * Use purple_account_get_string(irc->account, "quitmsg",
		 * IRC_DEFAULT_QUIT) and uncomment the appropriate account preference in
		 * irc.c if we decide we want custom quit messages.
		 */
		buf = irc_format(
			irc, "v:", "QUIT", (args && args[0]) ? args[0] : IRC_DEFAULT_QUIT
		);
		irc_priority_send(irc, buf);
		g_free(buf);
...
irc_do_mode(IRCConn *irc, const char *target, const char *sign, char **ops) {
	...
	while (ops[i]) {
		if (ops[i + 2] && ops[i + 4]) {
			...
			buf = irc_format(
				irc, "vcvnn", "MODE", target, mode, ops[i + 1], ops[i + 3]
			);
			i += 4;
		} else {
