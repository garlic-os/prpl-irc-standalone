...
static char *irc_send_convert(IRCConn *irc, const char *string);
static char *irc_recv_convert(IRCConn *irc, const char *string);

static void irc_parse_error_cb(IRCConn *irc, char *input);
...
/*typedef void (*IRCMsgCallback)(IRCConn *irc, char *from, char *name,
 * char **args);*/
typedef struct {
	const char *name;
	const char *format;
	...
	void (*cb)(IRCConn *irc, const char *name, const char *from, char **args);
} _IRCMsg;

static _IRCMsg _irc_msgs[] = {
	...
	{"authenticate",
	 ":",
	 1,
	 irc_msg_authenticate}, /* SASL authenticate		*/
#endif
...
static _IRCUserCmd _irc_cmds[] = {
	...
	{"away",
	 ":",
	 irc_cmd_away,
	 N_("away [message]:  Set an away message, or use no message to return "
		"from being away.")},
	{"ctcp",
	 "t:",
	 irc_cmd_ctcp,
	...
	{"deop",
	 ":",
	 irc_cmd_op,
	 N_("deop &lt;nick1&gt; [nick2] ...:  Remove channel operator status from "
		"someone. You must be a channel operator to do this.")},
	{"devoice",
	 ":",
	 irc_cmd_op,
	 N_("devoice &lt;nick1&gt; [nick2] ...:  Remove channel voice status from "
		"someone, preventing them from speaking if the channel is moderated "
		"(+m). You must be a channel operator to do this.")},
	{"invite",
	 ":",
	 irc_cmd_invite,
	 N_("invite &lt;nick&gt; [room]:  Invite someone to join you in the "
		"specified channel, or the current channel.")},
	{"j",
	 "cv",
	 irc_cmd_join,
	 N_("j &lt;room1&gt;[,room2][,...] [key1[,key2][,...]]:  Enter one or more "
		"channels, optionally providing a channel key for each if needed.")},
	{"join",
	 "cv",
	 irc_cmd_join,
	 N_("join &lt;room1&gt;[,room2][,...] [key1[,key2][,...]]:  Enter one or "
		"more channels, optionally providing a channel key for each if needed."
	 )},
	{"kick",
	 "n:",
	 irc_cmd_kick,
	 N_("kick &lt;nick&gt; [message]:  Remove someone from a channel. You must "
		"be a channel operator to do this.")},
	{"list",
	 ":",
	 irc_cmd_list,
	 N_("list:  Display a list of chat rooms on the network. <i>Warning, some "
	...
	{"mode",
	 ":",
	 irc_cmd_mode,
	 N_("mode &lt;+|-&gt;&lt;A-Za-z&gt; &lt;nick|channel&gt;:  Set or unset a "
		"channel or user mode.")},
	{"msg",
	 "t:",
	 irc_cmd_privmsg,
	 N_("msg &lt;nick&gt; &lt;message&gt;:  Send a private message to a user "
	...
	{"op",
	 ":",
	 irc_cmd_op,
	 N_("op &lt;nick1&gt; [nick2] ...:  Grant channel operator status to "
		"someone. You must be a channel operator to do this.")},
	{"operwall",
	 ":",
	 irc_cmd_wallops,
	 N_("operwall &lt;message&gt;:  If you don't know what this is, you "
	...
	{"part",
	 "c:",
	 irc_cmd_part,
	 N_("part [room] [message]:  Leave the current channel, or a specified "
		"channel, with an optional message.")},
	{"ping",
	 "n",
	 irc_cmd_ping,
	 N_("ping [nick]:  Asks how much lag a user (or the server if no user "
		"specified) has.")},
	{"query",
	 "n:",
	 irc_cmd_query,
	 N_("query &lt;nick&gt; &lt;message&gt;:  Send a private message to a user "
		"(as opposed to a channel).")},
	{"quit",
	 ":",
	 irc_cmd_quit,
	...
	{"remove",
	 "n:",
	 irc_cmd_remove,
	 N_("remove &lt;nick&gt; [message]:  Remove someone from a room. You must "
		"be a channel operator to do this.")},
	{"time",
	 "",
	 irc_cmd_time,
	...
	{"voice",
	 ":",
	 irc_cmd_op,
	 N_("voice &lt;nick1&gt; [nick2] ...:  Grant channel voice status to "
		"someone. You must be a channel operator to do this.")},
	{"wallops",
	 ":",
	 irc_cmd_wallops,
	 N_("wallops &lt;message&gt;:  If you don't know what this is, you "
		"probably can't use it.")},
	{"whois",
	 "tt",
	 irc_cmd_whois,
...
static PurpleCmdRet irc_parse_purple_cmd(
	...
	irc = (IRCConn *) gc->proto_data;

	if ((cmdent = (_IRCUserCmd *) g_hash_table_lookup(irc->cmds, cmd)) ==
		NULL) {
		return PURPLE_CMD_RET_FAILED;
	}

	(cmdent->cb
	)(irc, cmd, purple_conversation_get_name(conv), (const char **) args);

	return PURPLE_CMD_RET_OK;
}

static void irc_register_command(_IRCUserCmd *c) {
	char args[10];
	size_t i;

	PurpleCmdFlag f = (PurpleCmdFlag
	) (PURPLE_CMD_FLAG_CHAT | PURPLE_CMD_FLAG_IM | PURPLE_CMD_FLAG_PRPL_ONLY |
	   PURPLE_CMD_FLAG_ALLOW_WRONG_ARGS);

	const char *format = c->format;
...
static char *irc_send_convert(IRCConn *irc, const char *string) {
	...
	enclist = purple_account_get_string(
	...
	utf8 = g_convert(
		string, strlen(string), encodings[0], "UTF-8", NULL, NULL, &err
	);
	if (err) {
		purple_debug(
			PURPLE_DEBUG_ERROR,
			"irc",
			"Send conversion error: %s\n",
			err->message
		);
		purple_debug(
			PURPLE_DEBUG_ERROR,
...
static char *irc_recv_convert(IRCConn *irc, const char *string) {
	...
	autodetect = purple_account_get_bool(
		irc->account, "autodetect_utf8", IRC_DEFAULT_AUTODETECT
	);
...
char *irc_mirc2html(const char *string) {
	...
	do {
		...
		decoded = g_string_append_len(
			decoded, cur, (end ? (gssize) (end - cur) : (gssize) strlen(cur))
		);
		...
		switch (*cur) {
			...
			case '\003':
				...
				if (fg[0]) {
					...
					g_string_append_printf(
						decoded, "<FONT COLOR=\"%s\"", irc_mirc_colors[fgnum]
					);
					if (bg[0]) {
						bgnum = atoi(bg);
						if (bgnum >= 0 && bgnum < 16) {
							g_string_append_printf(
								decoded, " BACK=\"%s\"", irc_mirc_colors[bgnum]
							);
						}
					}
			...
			default:
				purple_debug(
					PURPLE_DEBUG_ERROR,
					"irc",
					"Unexpected mIRC formatting character %d\n",
					*cur
				);
		}
	} while (*cur);
...
const char *irc_nick_skip_mode(IRCConn *irc, const char *nick) {
...
char *irc_parse_ctcp(
	...
	if (!strncmp(cur, "ACTION ", 7)) {
		...
		if (notice) { /* reply */
			...
			if (sscanf(cur, "PING %lu", (unsigned long *) &timestamp) == 1) {
				buf = g_strdup_printf(
					_("Reply time from %s: %lu seconds"),
					from,
					time(NULL) - timestamp
				);
		...
		buf = irc_format(
			irc, "vt:", "NOTICE", from, "\001VERSION Purple IRC\001"
		);
		irc_send(irc, buf);
		g_free(buf);
...
void irc_msg_table_build(IRCConn *irc) {
	...
	for (i = 0; _irc_msgs[i].name; i++) {
		g_hash_table_insert(
			irc->msgs, (gpointer) _irc_msgs[i].name, (gpointer) &_irc_msgs[i]
		);
	}
}
...
void irc_cmd_table_build(IRCConn *irc) {
	...
	for (i = 0; _irc_cmds[i].name; i++) {
		g_hash_table_insert(
			irc->cmds, (gpointer) _irc_cmds[i].name, (gpointer) &_irc_cmds[i]
		);
	}
}
...
void irc_parse_msg(IRCConn *irc, char *input) {
	...
	if (!strncmp(input, "PING ", 5)) {
		...
		if (g_utf8_validate(input, -1, NULL)) {
			...
			purple_connection_error_reason(
				gc, PURPLE_CONNECTION_ERROR_NETWORK_ERROR, tmp
			);
			g_free(tmp);
		} else {
			purple_connection_error_reason(
				gc, PURPLE_CONNECTION_ERROR_NETWORK_ERROR, _("Disconnected.")
			);
		}
		return;
#ifdef HAVE_CYRUS_SASL
	...
	if ((msgent = (_IRCMsg *) g_hash_table_lookup(irc->msgs, msgname)) ==
		NULL) {
	...
	args_cnt = 0;
	for (cur = end, fmt = (char *) msgent->format, i = 0; fmt[i] && *cur++;
		 i++) {
		switch (fmt[i]) {
			case 'v':
				if (!(end = strchr(cur, ' '))) {
...
static void irc_parse_error_cb(IRCConn *irc, char *input) {
	...
	purple_debug(
		PURPLE_DEBUG_WARNING, "irc", "Unrecognized string: %s\n", clean
	);
	g_free(clean);
}
