...
static void irc_ison_buddy_init(char *name, IRCBuddy *ib, GList **list);
...
static void irc_ssl_connect_failure(
	PurpleSslConnection *gsc,
	PurpleSslErrorType error,
	gpointer data
);
static void irc_close(PurpleConnection *gc);
static int irc_im_send(
	PurpleConnection *gc,
	const char *who,
	const char *what,
	PurpleMessageFlags flags
);
static int irc_chat_send(
	PurpleConnection *gc,
	int id,
	const char *what,
	PurpleMessageFlags flags
...
static void IRCBuddy_free(IRCBuddy *ib);
...
static void irc_view_motd(PurplePluginAction *action) {
	...
	body = g_strdup_printf(
		"<span style=\"font-family: monospace;\">%s</span>", irc->motd->str
	);
...
static int do_send(IRCConn *irc, const char *buf, gsize len) {
...
int irc_priority_send(IRCConn *irc, const char *buf) {
	if (irc->sent_partial) {
		g_queue_insert_after(
			irc->send_queue, irc->send_queue->head, g_strdup(buf)
		);
		return 0;
	} else {
...
static int irc_send_raw(PurpleConnection *gc, const char *buf, int len) {
	IRCConn *irc = (IRCConn *) gc->proto_data;
	if (len == -1) {
		len = strlen(buf);
	}
...
static gboolean irc_send_handler_cb(gpointer data) {
	IRCConn *irc = (IRCConn *) data;
	gint available = 0;
	gint interval = 0;

	interval = purple_account_get_int(
		irc->account, "ratelimit-interval", IRC_DEFAULT_COMMAND_INTERVAL
	);
	...
	if (interval < 1) {
		...
		gint burst = purple_account_get_int(
			irc->account, "ratelimit-burst", IRC_DEFAULT_COMMAND_MAX_BURST
		);
...
void irc_send_len(IRCConn *irc, const char *buf, int buflen) {
	(void) buflen;
...
void IRCBuddy_query(IRCConn *irc) {
	...
	while ((lp = g_list_first(irc->buddies_outstanding))) {
		...
		irc->buddies_outstanding =
			g_list_remove_link(irc->buddies_outstanding, lp);
	}
...
static void irc_ison_one(IRCConn *irc, IRCBuddy *ib) {
	char *buf;
...
irc_chat_info_defaults(PurpleConnection *gc, const char *chat_name) {
	...
	if (chat_name != NULL) {
		g_hash_table_insert(defaults, (char *) "channel", g_strdup(chat_name));
	}
...
static void irc_login(PurpleAccount *account) {
	...
	gc->flags =
		(PurpleConnectionFlags) (gc->flags | PURPLE_CONNECTION_NO_NEWLINES);
	...
	gc->proto_data = irc = g_new0(IRCConn, 1);
	irc->fd = -1;
	irc->account = account;
	...
	irc->buddies = g_hash_table_new_full(
		(GHashFunc) irc_nick_hash,
		(GEqualFunc) irc_nick_equal,
		NULL,
		(GDestroyNotify) IRCBuddy_free
	);
...
irc_login_cb(gpointer data, gint source, const gchar *error_message) {
	...
	if (do_login(gc)) {
		gc->inpa =
			purple_input_add(irc->fd, PURPLE_INPUT_READ, irc_input_cb, gc);
	}
}
...
irc_add_buddy(PurpleConnection *gc, PurpleBuddy *buddy, PurpleGroup *group) {
	...
	ib = (IRCBuddy *) g_hash_table_lookup(irc->buddies, bname);
	if (ib != NULL) {
		ib->ref++;
		purple_prpl_got_user_status(
			irc->account, bname, ib->online ? "available" : "offline", NULL
		);
	} else {
		ib = g_new0(IRCBuddy, 1);
...
irc_remove_buddy(PurpleConnection *gc, PurpleBuddy *buddy, PurpleGroup *group) {
	...
	ib = (IRCBuddy *) g_hash_table_lookup(
		irc->buddies, purple_buddy_get_name(buddy)
	);
	if (ib && --ib->ref == 0) {
...
static void read_input(IRCConn *irc, int len) {
	char *cur, *end;
	...
	do {
		...
		if (irc->inbuflen < irc->inbufused + IRC_BUFSIZE_INCREMENT) {
			if (irc->inbuflen + IRC_BUFSIZE_INCREMENT <= IRC_MAX_BUFSIZE) {
				...
				irc->inbuf = (char *) g_realloc(irc->inbuf, irc->inbuflen);
			} else {
		...
		len = purple_ssl_read(
			gsc, irc->inbuf + irc->inbufused, irc->inbuflen - irc->inbufused - 1
		);
		if (len > 0) {
			read_input(irc, len);
		}
...
irc_input_cb(gpointer data, gint source, PurpleInputCondition cond) {
	...
	len = read(
		irc->fd, irc->inbuf + irc->inbufused, irc->inbuflen - irc->inbufused - 1
	);
...
static void irc_chat_join(PurpleConnection *gc, GHashTable *data) {
	...
	args[0] = (char *) g_hash_table_lookup(data, "channel");
	args[1] = (char *) g_hash_table_lookup(data, "password");
	irc_cmd_join(irc, "join", NULL, args);
}

static char *irc_get_chat_name(GHashTable *data) {
	return (char *) g_strdup((char *) g_hash_table_lookup(data, "channel"));
}

static void irc_chat_invite(
	PurpleConnection *gc,
	int id,
	const char *message,
	const char *name
...
static int irc_chat_send(
	...
	serv_got_chat_in(
		gc, id, purple_connection_get_display_name(gc), flags, what, time(NULL)
	);
	g_free(tmp);
	return 0;
}
...
static PurpleRoomlist *irc_roomlist_get_list(PurpleConnection *gc) {
	...
	f = purple_roomlist_field_new(
		PURPLE_ROOMLIST_FIELD_STRING, "", "channel", TRUE
	);
	fields = g_list_append(fields, f);

	f = purple_roomlist_field_new(
		PURPLE_ROOMLIST_FIELD_INT, _("Users"), "users", FALSE
	);
	fields = g_list_append(fields, f);

	f = purple_roomlist_field_new(
		PURPLE_ROOMLIST_FIELD_STRING, _("Topic"), "topic", FALSE
	);
	fields = g_list_append(fields, f);
...
PurpleBuddyIconSpec no_buddy_icons =
	{NULL, 0, 0, 0, 0, 0, (PurpleIconScaleRules) 0};
...
static PurplePluginInfo info = {
	...
	(char *) N_("IRC Protocol Plugin"), /**  summary        */
	(char *) N_("The IRC Protocol Plugin that Sucks Less"), /**  description */
	NULL,					 /**< author         */
...
static void _init_plugin(PurplePlugin *plugin) {
	...
	option = purple_account_option_string_new(
		_("Encodings"), "encoding", IRC_DEFAULT_CHARSET
	);
	prpl_info.protocol_options =
		g_list_append(prpl_info.protocol_options, option);

	option = purple_account_option_bool_new(
		_("Auto-detect incoming UTF-8"),
		"autodetect_utf8",
	...
	IRC_DEFAULT_QUIT); prpl_info.protocol_options =
	g_list_append(prpl_info.protocol_options, option);
	*/
...
#ifdef HAVE_CYRUS_SASL
	option = purple_account_option_bool_new(
		_("Authenticate with SASL"), "sasl", FALSE
	);
	prpl_info.protocol_options =
		g_list_append(prpl_info.protocol_options, option);

	option =
		purple_account_option_string_new(_("SASL login name"), "saslname", "");
	prpl_info.protocol_options =
		g_list_append(prpl_info.protocol_options, option);
	...
	option = purple_account_option_int_new(
		_("Seconds between sending messages"),
		"ratelimit-interval",
	...
	option = purple_account_option_int_new(
		_("Maximum messages to send at once"),
		"ratelimit-burst",
