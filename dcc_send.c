...
static void irc_dccsend_recv_destroy(PurpleXfer *xfer) {
	// "just trust us, we're tim ringenbach and robbert haarman"
	IRCXferRXData *xd = (IRCXferRXData *) xfer->data;
		...
		if (!strchr(&(token[0][1]), '"')) {
			...
			for (i = 1; token[i]; i++) {
				if (!strchr(token[i], '"')) {
					...
					g_string_append_len(
						filename, token[i], strlen(token[i]) - 1
					);
					break;
				}
			}
			...
			g_string_append_len(
				filename, &(token[0][1]), strlen(&(token[0][1])) - 1
			);
		}
	} else {
...
static void irc_dccsend_send_destroy(PurpleXfer *xfer) {
	IRCXferSendData *xd = (IRCXferSendData *) xfer->data;
...
irc_dccsend_send_read(gpointer data, int source, PurpleInputCondition cond) {
	...
	xd->rxqueue = (guchar *) g_realloc(xd->rxqueue, len + xd->rxlen);
	...
	while (1) {
		...
		xd->rxlen -= 4;
		if (xd->rxlen) {
			guchar *tmp = (guchar *) g_memdup2(xd->rxqueue + 4, xd->rxlen);
			g_free(xd->rxqueue);
			xd->rxqueue = tmp;
		} else {
			g_free(xd->rxqueue);
...
static void irc_dccsend_send_connected(
	gpointer data,
	int source,
	...
	xd->inpa =
		purple_input_add(conn, PURPLE_INPUT_READ, irc_dccsend_send_read, xfer);
	/* Start the transfer */
	purple_xfer_start(xfer, conn, NULL, 0);
}

static void irc_dccsend_network_listen_cb(int sock, gpointer data) {
	PurpleXfer *xfer = (PurpleXfer *) data;
	IRCXferSendData *xd;
	PurpleConnection *gc;
	IRCConn *irc;
	...
	xfer->watcher = purple_input_add(
		sock, PURPLE_INPUT_READ, irc_dccsend_send_connected, xfer
	);
	...
	arg[1] = tmp = g_strdup_printf(
		"\001DCC SEND \"%s\" %u %hu %" G_GSIZE_FORMAT "\001",
		xfer->filename,
		ntohl(addr.s_addr),
		port,
		xfer->size
	...
	irc_cmd_privmsg(irc, "msg", NULL, arg);
	g_free(tmp);
}
...
static void irc_dccsend_send_init(PurpleXfer *xfer) {
	...
	xd->listen_data = purple_network_listen_range(
		0, 0, SOCK_STREAM, irc_dccsend_network_listen_cb, xfer
	);
	if (xd->listen_data == NULL) {
		purple_xfer_unref(xfer);
		purple_notify_error(
...
PurpleXfer *irc_dccsend_new_xfer(PurpleConnection *gc, const char *who) {
	...
	xfer = purple_xfer_new(
		purple_connection_get_account(gc), PURPLE_XFER_SEND, who
	);
	if (xfer) {
		xd = g_new0(IRCXferSendData, 1);
		xd->fd = -1;
		xfer->data = xd;
